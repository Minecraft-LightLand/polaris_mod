...
public class AutoAim {
    ...
    private static ProjectileEntity getProcessedPE(ShootConfig config) {
        ...
        Estimator.EstiResult er = setAim(config.player, config.velo, config.r, e, config.g, config.k, config.t);
    ...
    private static Entity getTNTEntity(ShootConfig config) {
        ...
        Estimator.EstiResult er = setAim(config.player, config.velo, 128, e, 0.04, 0.02, 80);
        if (er.getType() == Estimator.EstiType.ZERO) {
            e.setDeltaMovement(er.getVec());
            e.setFuse((int) Math.round(er.getT()));
        } else if (er.getType() == Estimator.EstiType.FAIL)
    ...
    private static Estimator.EstiResult setAim(PlayerEntity pl, double velo, double reach, Entity e, double g, double k,
                                               int maxt) {
        ...
        if (ertr != null && ertr.getType() == EntityRayTraceResult.Type.ENTITY) {
            ...
            LogManager.getLogger().info("aim status success: " + (er.getType() == Estimator.EstiType.ZERO));
            if (er.getType() == Estimator.EstiType.ZERO)
                return er;
        }
